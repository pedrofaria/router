...
This library is a modification of [nikic/FastRoute][base_lib] implementation.
...
The routes are defined by calling the `Router\simpleDispatcher()` function, which accepts
a callable taking a `Router\RouteCollector` instance. The routes are added by calling
...
class name or any other kind of data you wish to associate with the route. Router only tells you
...
Router\Exception\HttpNotFoundException or Router\Exception\HttpMethodNotAllowedException.
...
`Allow:` header to detail available methods for the requested resource. Applications using Router
...
However, implementers using Router outside the web SAPI environment (e.g. a custom server) MUST
NOT send entity bodies generated in response to HEAD requests. If you are a non-SAPI user this is
*your responsibility*; Router has no purview to prevent you from breaking HTTP in such cases.
...
[base_lib]: https://github.com/nikic/FastRoute